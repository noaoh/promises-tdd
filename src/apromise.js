...
function handle(promise, handler) {
  ...
  while(promise.value instanceof APromise && promise.state !== PROMISE_STATES.REJECTED) {
    promise = promise.value
  }

  if(promise.state === PROMISE_STATES.PENDING) {
    // queue if PROMISE_STATES.PENDING
...
function handleResolved(promise, handler) {
  setImmediate(() => {
    const cb = promise.state === PROMISE_STATES.FULFILLED ? handler.onFulfilled : handler.onRejected
    if(typeof cb !== 'function') {
      if(promise.state === PROMISE_STATES.FULFILLED) {
...
function fulfill(promise, value) {
  ...
  if(value &&(typeof value === 'object' || typeof value === 'function')) {
    ...
    if(then === promise.then && promise instanceof APromise) {
      promise.state = PROMISE_STATES.FULFILLED
      promise.value = value
...
function reject(promise, reason) {
  promise.state = PROMISE_STATES.REJECTED
...
export { APromise, PROMISE_STATES }
